...
try {
    ...
    if ($exists_stmt->rowCount() > 0) {
        ...
        $update_query = "UPDATE product_suppliers 
                        SET is_primary = :is_primary, price = :price, updated_at = NOW() 
                        WHERE id = :id";
        $update_stmt = $db->prepare($update_query);
        ...
        $insert_query = "INSERT INTO product_suppliers (product_id, contact_id, is_primary, price) 
                        VALUES (:product_id, :contact_id, :is_primary, :price)";
